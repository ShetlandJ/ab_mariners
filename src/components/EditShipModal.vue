  ...
  <div v-if="show" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold dark:text-white">{{ isCreating ? 'Create New Ship' : 'Edit Ship' }}</h2>
      ...
      <form @submit.prevent="saveChanges">
        ...
        <div class="form-group mb-4">
          ...
          <input
            ...
            type="text"
            class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            required
          />
        </div>
        ...
        <div class="mt-6 flex justify-end space-x-3">
          ...
          <button
            ...
            :disabled="isSaving"
          >
            {{ isSaving ? 'Saving...' : isCreating ? 'Create Ship' : 'Save Changes' }}
          </button>
        </div>
      </form>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Check if we're creating a new ship by looking for empty object
    const isCreating = computed(() => {
      return !props.ship.shipID;
    });
    ...
    async function saveChanges() {
      try {
        ...
        if (isCreating.value) {
          // Create a new ship
          updatedShip = await database.createShip(editedShip.value);
        } else {
          // Update existing ship
          updatedShip = await database.updateShip(editedShip.value);
        }
        
        emit('saved', updatedShip);
        emit('close');
      } catch (error) {
        console.error(`Error ${isCreating.value ? 'creating' : 'updating'} ship:`, error);
        alert(`Failed to ${isCreating.value ? 'create' : 'update'} ship. Please try again.`);
      } finally {
